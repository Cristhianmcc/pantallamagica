...
import { getGoogleDriveStreamUrl, getGoogleDriveEmbedUrl, getGoogleDriveFile } from '../../../../utils/googledrive';
...
export async function GET(request, { params }) {
  ...
  try {
    ...
    if (process.env.USE_GOOGLE_DRIVE === 'true' && process.env.GOOGLE_DRIVE_API_KEY) {
      try {
        // Validar que el archivo existe en Google Drive
        const fileInfo = await getGoogleDriveFile(movieId);
    ...
    return NextResponse.json(
      { success: false, error: 'Error al obtener el enlace de streaming' },
      { status: 500 }
    );
  }
}
